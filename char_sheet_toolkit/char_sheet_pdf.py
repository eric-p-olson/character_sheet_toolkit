    ...
    if "/Kids" in field:
        for k in field["/Kids"]:
            ...
            if "/Opt" in kid and isinstance(kid["/Opt"], ArrayObject):
                ...
                "/AP" in kid
                and "/N" in kid["/AP"]
                and isinstance(kid["/AP"]["/N"], DictionaryObject)
            ):
...
class CharSheetPdf:
    ...
    def apply_form_properties(self, pc_data: PCData):
        ...
        if os.path.isfile(os.path.join(form_path, form_properties_file + ".py")):
            ...
            print(
                "Debug - no form properties file: {os.path.join(form_path, form_properties_file)}"
            )
    ...
    def create_fdf(self, pc_data: PCData, pdf_filename):
        ...
        shutil.copy(
            self.doc_name, pdf_output_file
        )  # create copy of master for fdd to overwrite

        fdf_template = os.path.join(_location, "fdf.mako")
        fdf_txt = mako_render(
            fdf_template,
            pc_data.character_data(),
    ...
    def create_mapping(self, output_mapping_file):
        """Create .map.yaml file for a pdf -
           this allows any field property to be overwritten
        """
        field_map = {}
        fields = self.reader.get_fields()
        if fields is None:
            ...
            if field.get("/FT") == "/Btn":  # checkbox
                ...
                if mapped_field_name and mapped_field_name != "?":
                    if field_value != mapped_field_name:
                        print(
                            f"Text {field_name} updated '{mapped_field_name}' -> '{field_value}'"
                        )
                    field_map[str(field_name)] = {
                        "T": field_value,
                        "DV": "${" + field_value + "}",
                    }
                elif field_value:
